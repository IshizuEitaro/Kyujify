...
async function loadVariantGroupsFromFile(
  filePath: string,
  ...
  try {
    ...
    console.log(
      `[Kyujify] Successfully loaded ${normalized.length} variant groups from ${filePath}`,
    );
    return normalized;
  } catch (error: any) {
...
export async function getNextVariantMap(
  context: vscode.ExtensionContext,
...
async function loadConversionPairsFromFile(
  filePath: string,
  context: vscode.ExtensionContext,
...
export async function getConversionPairs(
  ...
  if (customPath !== defaultPath) {
    vscode.window.showWarningMessage(
      `Failed to load conversion pairs from '${customPath}'. Falling back to default pairs.`,
    );
  ...
  vscode.window.showErrorMessage(
    "Failed to load any conversion pairs. Please check the extension settings and file integrity.",
  );
  console.error("[Kyujify] FATAL: Failed to load any conversion pairs.");
  return [];
}

async function loadKakikaeRulesFromFile(
  filePath: string,
  context: vscode.ExtensionContext,
): Promise<KakikaeRule[] | null> {
...
export async function getKakikaeMap(
  settings: vscode.WorkspaceConfiguration,
  context: vscode.ExtensionContext,
